...
export class ObjectVrakkaNFT{
    ...
    async load(){
        ...
        let winner=await determinarChain(deployedNetwork,0)
          ...
          const contrato=new web3.eth.Contract(
            VrakkaJson.abi, // Llama el contrato ABI
            winner
            )
    ...
    async mint(price,URI_Image){
        ...
        try {
            var _res=await this.contrato.methods.PublicMinting(URI_Image).send({from:this.account,value:weiPrice})
            return _res.status
        } catch (error) {
    ...
    async changeSaleState(_index){
        ...
        try {
            var _res=await this.contrato.methods.toggleForSale(_index).send({from:this.account})
            return _res.status
        } catch (error) {
            return false
        }
    ...
    async changePrice(_index,_currentPrice){
        ...
        try {
            var _res=await this.contrato.methods.changePrice(_index,wei_price).send({from:this.account})
            return _res.status
        } catch (error) {
    ...
    async changePriceAndState(_index,_currentPrice){
        // El input ya viene en wei, no es necesario convertirlo
        try {
            var _res=await this.contrato.methods.toggleForSaleAndPrice(_index,_currentPrice).send({from:this.account})
            return _res.status
        } catch (error) {
    ...
    async fillMarket(){
        ...
        for (let index = 0; index < _total+1; index++) {
            ...
            try {
                ...
                if(vrakka_data['currentOwner']!='0x0000000000000000000000000000000000000000'){
