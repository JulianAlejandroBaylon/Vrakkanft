...
export class ObjectToken{
    ...
    async claim(tokenId){
        try {
            var _res=await this.contrato.methods.claimSupply(tokenId).send({from:this.account})
            return _res.status
        } catch (error) {
    ...
    async deposit(tokenId){
        try {
            var _res=await this.contrato.methods.deposit(tokenId).send({from:this.account})
            return _res.status
        } catch (error) {
